...
public class CSVReader : MonoBehaviour 
{
	...
	// splits a CSV file into a 2D string array
	static public string[][] SplitCsvJaggedGrid(string csvText)
	{
		string[] lines = csvText.Replace("\r\n", "\n").Replace("\n\r", "\n").Split("\n"[0]);
//		string[] lines = csvText.Split("\n"[0]); 

		// creates new 2D string grid to output to
		string[][] outputJagged = new string[lines.Length][];
		for (int y = 0; y < lines.Length; y++)
		{
			outputJagged[y] = SplitCsvNewLine(lines[y]);
		}
		return outputJagged; 
	}
	...
	static public string[] SplitCsvNewLine(string line)
	{
		return (from System.Text.RegularExpressions.Match m in System.Text.RegularExpressions.Regex.Matches(line,@"(((?<x>(?=[,\n]+))|""(?<x>([^""]|"""")+)""|(?<x>[^,\n]+)),?)",System.Text.RegularExpressions.RegexOptions.ExplicitCapture) select m.Groups[1].Value).ToArray();
	}
}